...
const envSchema = z.object({
  ...
  SPLUNK_SEND_CURVE: z.coerce.boolean().default(false),
  SPLUNK_SEND_CURVE_SUMMARY: z.coerce.boolean().default(true),
  SPLUNK_CURVE_SAMPLE_INTERVAL_SEC: z.coerce.number().positive().default(1),
  SPLUNK_CURVE_SAMPLE_MAX_POINTS: z.coerce.number().int().nonnegative().default(120),
  ...
  SPLUNK_STREAM_POINTS_MODE: z.enum(['full', 'sampled', 'none']).default('full'),
  SPLUNK_STREAM_POINTS_MAX: z.coerce.number().int().nonnegative().default(5000),
  SPLUNK_INCLUDE_RAW_STREAM: envBool(false),
  SPLUNK_RAW_STREAM_MAX: z.coerce.number().int().nonnegative().default(1000),
