...
export class LpcTcpClient extends EventEmitter {
  ...
  private testStartCommandAt: string | null = null;
  ...
  markTestStartCommand(): void { this.testStartCommandAt = new Date().toISOString(); this.streamingHealthy = false; this.lastStreamFrameAt = null; this.lastResultFrameAt = null; this.operatorMessage = this.usingFallbackInterface ? this.operatorMessage : null; this.clearStreamWatchdog(); console.log(`[LPC] test start sent interface=${this.selectedInterface ?? this.currentInterfaceAttempt ?? this.options.preferredInterface}`); console.log(`[LPC] waiting for stream/result watchdogMs=${this.options.streamWatchdogMs}`); this.streamWatchdogTimer = setTimeout(() => { if (this.lastStreamFrameAt || this.lastResultFrameAt) return; this.streamingHealthy = false; this.lpcStatusCode = 'LPC_STREAM_STALLED'; this.operatorMessage = 'Zresetuj LPC, następnie IPC.'; console.warn(`[LPC] stream watchdog expired interface=${this.selectedInterface ?? this.currentInterfaceAttempt ?? this.options.preferredInterface}`); console.warn('[LPC] connection incomplete: TCP/interface OK but no S/R frames'); console.warn('[LPC] operator action required: reset LPC then IPC'); this.emitStatus(); }, this.options.streamWatchdogMs); this.emitStatus(); }
  recordStreamFrame(kind: 'stream' | 'result', receivedAt = new Date().toISOString()): void { this.lastLpcRxAt = receivedAt; this.streamingHealthy = true; if (this.lpcStatusCode === 'LPC_STREAM_STALLED') this.lpcStatusCode = this.usingFallbackInterface ? 'LPC_CONNECTED_FALLBACK_INTERFACE' : null; if (kind === 'stream') { this.lastStreamFrameAt = receivedAt; console.log(`[LPC] first stream frame received interface=${this.selectedInterface ?? this.options.preferredInterface}`); } else { this.lastResultFrameAt = receivedAt; console.log(`[LPC] result frame received interface=${this.selectedInterface ?? this.options.preferredInterface}`); } console.log(`[LPC] streaming healthy interface=${this.selectedInterface ?? this.options.preferredInterface}`); this.clearStreamWatchdog(); this.emitStatus(); }
  canStartTest(): { ok: true } | { ok: false; code: string; message: string } { if (this.lpcStatusCode === 'LPC_NO_AVAILABLE_INTERFACE') return { ok: false, code: 'LPC_NO_AVAILABLE_INTERFACE', message: this.operatorMessage ?? 'Zresetuj LPC, następnie IPC.' }; if (this.lpcStatusCode === 'LPC_INTERFACE_UNAVAILABLE') return { ok: false, code: 'LPC_INTERFACE_UNAVAILABLE', message: `Interface LPC ${this.lastInterfaceAttempt ?? this.options.preferredInterface} jest niedostępny. Prawdopodobnie jest zajęty przez inną sesję lub poprzednie połączenie nie zostało zwolnione.` }; if (this.lpcStatusCode === 'LPC_STREAM_STALLED') return { ok: false, code: 'LPC_STREAM_STALLED', message: this.operatorMessage ?? 'Zresetuj LPC, następnie IPC.' }; if (!this.state.isConnected() || !this.interfaceSelected) return { ok: false, code: 'LPC_NOT_READY', message: `LPC nie jest gotowy: TCP lub Interface ${this.options.preferredInterface} nie zostały poprawnie zestawione.` }; return { ok: true }; }
