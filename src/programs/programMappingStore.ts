...
export function isLabelPrintMode(value: unknown): value is LabelPrintMode {
  return value === 'ok_only' || value === 'ok_and_nok' || value === 'disabled';
}
...
function validateProgramMappingInput(input: ProgramMappingInput, partial = false): {
  ...
  if (!partial || input.labelPrintMode !== undefined) {
    ...
    if (!isLabelPrintMode(labelPrintMode)) throw new Error('Tryb drukowania etykiet jest wymagany.');
