...
export function createLlControlRouter(options: { database: AppDatabase; splunkBuffer?: SplunkBuffer; splunkConfig?: SplunkRuntimeConfig }): Router {
  ...
  router.post('/flag', async (req: AuthenticatedRequest, res) => {
    ...
    console.log(`[LL_CONTROL] POST /flag barcode=${barcode || '-'} testId=${testId || '-'} user=${req.user?.login ?? '-'}`);
    if (!barcode || !testId) {
      console.log('[LL_CONTROL] rejected reason=INVALID_REQUEST');
      return res.status(400).json({ ok: false, code: 'INVALID_REQUEST', error: 'INVALID_REQUEST', message: 'Nie można oznaczyć sztuki — brak barcode lub ID testu.' });
    }
    const result = options.database.findTestResultById(testId);
    if (!result || result.barcode !== barcode) {
      console.log('[LL_CONTROL] rejected reason=TEST_NOT_FOUND');
      return res.status(404).json({ ok: false, code: 'TEST_NOT_FOUND', error: 'TEST_NOT_FOUND', message: 'Nie znaleziono wskazanego wyniku testu.' });
    }
    if (!requiresLlControl(result.result)) {
      console.log(`[LL_CONTROL] rejected reason=RESULT_NOT_ELIGIBLE result=${result.result}`);
      return res.status(400).json({ ok: false, code: 'RESULT_NOT_ELIGIBLE_FOR_LL_CONTROL', error: 'RESULT_NOT_ELIGIBLE_FOR_LL_CONTROL', message: 'Kontrola LL może zostać oznaczona tylko po NOK/REJECT/UNKNOWN.' });
    }
    const existing = options.database.findOpenLlControlFlag(barcode);
    if (existing) {
      console.log(`[LL_CONTROL] existing barcode=${barcode} flagId=${existing.id}`);
      return res.json({ ok: true, created: false, existing: true, flag: existing });
    }
    ...
    console.log(`[LL_CONTROL] created barcode=${barcode} flagId=${flag.id}`);
    emitLlFlagged(options.splunkBuffer, options.splunkConfig, flag); res.json({ ok: true, created: true, existing: false, flag });
  });
