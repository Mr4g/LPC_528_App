...
const baseConfig: SplunkRuntimeConfig = { enabled: true, url: 'http://127.0.0.1/services/collector', token: 'token-for-test', index: 'machinedata_w16', source: 'LPC-528-01', sourcetype: '_json', timeoutMs: 1000, verifyTls: true, sendResult: true, sendCurve: true, bufferEnabled: true, bufferRetryIntervalMs: 30000, bufferMaxAttempts: 0 };
...
function listen(handler: http.RequestListener): Promise<{ url: string; requests: { url?: string; body: unknown }[] }> {
...
describe('SplunkClient', () => {
  ...
  it('sends a single HEC envelope JSON body to /services/collector/event', async () => {
    ...
    expect(server.requests[0]).toMatchObject({ url: '/services/collector/event', body: { index: 'machinedata_w16', source: 'LPC-528-01', sourcetype: '_json', event: { eventType: 'lpc_test_result' } } });
  });
