...
function getAutoPrintSetting(options: { zebraService: ZebraService; database?: AppDatabase }) {
  const enabledFromEnv = options.zebraService.getStatus().printOnResult;
  const stored = options.database?.getAppSetting('zebra.autoPrintEnabled');
  const autoPrintEnabled = stored ? stored.value !== 'false' : enabledFromEnv;
  return { enabledFromEnv, autoPrintEnabled, effectiveAutoPrintEnabled: enabledFromEnv && autoPrintEnabled };
}
...
export function createZebraRouter(options: { zebraService: ZebraService; lastResultStore: LastResultStore; resultHistoryStore: ResultHistoryStore; database?: AppDatabase }): Router {
  ...
  router.get('/settings', requireRole(['admin', 'line_leader']), (_req, res) => {
    res.json({ ok: true, ...getAutoPrintSetting(options) });
  });

  router.patch('/settings', requireRole(['admin', 'line_leader']), (req: AuthenticatedRequest, res) => {
    if (typeof req.body?.autoPrintEnabled !== 'boolean') {
      return res.status(400).json({ ok: false, error: 'INVALID_AUTO_PRINT_ENABLED', message: 'autoPrintEnabled musi być boolean.' });
    }
    options.database?.setAppSetting('zebra.autoPrintEnabled', req.body.autoPrintEnabled ? 'true' : 'false', req.user?.login ?? null);
    return res.json({ ok: true, ...getAutoPrintSetting(options) });
  });
